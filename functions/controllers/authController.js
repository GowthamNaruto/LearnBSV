...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!user || !(await user.correctPassword(password, user.password))) {
    return next(new AppError('Incorrect emali or password', 401));
  }
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(
      new AppError(
        'The user bolongong to this token does no longer exist.',
        401
      )
    );
  }
  ...
  //  GTANT ACCESS TO PROCETED ROUTE
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Update chandedPassword  property for the user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // const passwordCurrent = await user.password;
